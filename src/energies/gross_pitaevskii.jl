...
struct GrossPitaevskiiRayleighQuotient{
  T,MK<:AbstractMatrix{T},MM<:AbstractMatrix{T},FQ,FG,FD
} <: AbstractEnergy{T}
  K::MK
  M::MM
  beta::T
...
function GrossPitaevskiiRayleighQuotient(
  ...
  return GrossPitaevskiiRayleighQuotient{
    ...
    K, M, beta_T, quartic, cubic_gradient, density_matrix
  )
end
...
struct GrossPitaevskiiTangentQuadraticModel{
  T,MH<:AbstractMatrix{T},MM<:AbstractMatrix{T},V<:AbstractVector{T}
} <: AbstractEnergy{T}
  H::MH
  M::MM
  u::V
...
function tangent_quadratic_model(
  ...
  isnothing(e.density_matrix) && throw(ArgumentError(
    "tangent_quadratic_model requires a density_matrix callback",
  ))
  length(u) == dimension(e) || throw(DimensionMismatch(
    "iterate length must equal the energy dimension",
  ))
  u_normalized = copy(u)
  normalize_M!(u_normalized, e.M)
  mu = chemical_potential(e, u_normalized)
  residual = e.K * u_normalized .+
    e.beta .* e.cubic_gradient(u_normalized) .- mu .* (e.M * u_normalized)
  H = e.K + 3e.beta .* e.density_matrix(u_normalized)
  return GrossPitaevskiiTangentQuadraticModel(
    H, e.M, u_normalized, residual, mu
  )
end
...
function frozen_density_model(
  ...
  isnothing(e.density_matrix) && throw(ArgumentError(
    "frozen_density_model requires a density_matrix callback",
  ))
  length(u) == dimension(e) || throw(DimensionMismatch(
    "iterate length must equal the energy dimension",
  ))
  0 < alpha <= 1 || throw(ArgumentError("alpha must satisfy 0 < alpha <= 1"))
  !isnothing(previous) && length(previous) != dimension(e) && throw(
    DimensionMismatch("previous iterate length must equal the energy dimension"),
  )
  u_normalized = copy(u)
  normalize_M!(u_normalized, e.M)
  ...
  if !isnothing(previous) && alpha != 1
    ...
    density = alpha .* density .+
      (1 - alpha) .* e.density_matrix(previous_normalized)
  end
